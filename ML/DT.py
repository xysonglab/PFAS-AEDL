"""
Decision Tree Model Training with Optuna Optimization and Feature Importance Analysis
...
class DecisionTreeModelTrainer:
    """Decision Tree模型训练器,包含Optuna优化、交叉验证和特征重要性分析"""
    ...
    def optuna_optimization(self, n_trials: int = 20) -> Dict[str, Any]:
        ...
        self.study = optuna.create_study(
            direction='minimize',
            sampler=sampler,
            study_name='decision_tree_hyperparameter_optimization'
        )
    ...
    def train_final_model(self, params: Dict[str, Any]) -> None:
        ...
        # 训练模型
        self.best_model = DecisionTreeRegressor(**params)
    ...
    def save_model(self) -> None:
        ...
        # 保存模型
        model_path = os.path.join(self.output_dir, 'decision_tree_model.pkl')
    ...
    def run_full_pipeline(
        ...
        """
        print("=" * 60)
        print("Decision Tree模型训练流程 (含特征重要性分析)")
        print("=" * 60)
        ...
        print(f"\n输出文件清单:")
        print(f"  - decision_tree_model.pkl       : 训练好的模型")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Decision Tree模型训练 (Optuna优化 + 特征重要性分析)')
    ...
    parser.add_argument('--output_dir', type=str, default='./results/DT',
                        help='输出目录 (默认: ./results/DT)')
    ...
    # 创建训练器并运行
    trainer = DecisionTreeModelTrainer(
